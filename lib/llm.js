// Claude API wrapper — 統一處理 JSON 模式呼叫 + 用量追蹤
import Anthropic from '@anthropic-ai/sdk';
import { trackAnthropic } from './infuz-usage.js';
 ...
 * @param {object} opts
 * @param {string} [opts.model] - 覆寫預設 model (省錢: 長文可傳 'claude-haiku-4-5')
 */
export async function callJSON({ system, user, maxTokens = 8000, temperature = 0.9, endpoint, model }) {
  ...
  // 用量追蹤 — await 防 serverless 結束時 kill (追蹤實際 model, 不是 default)
  if (resp.usage) {
    try {
      await trackAnthropic({
        ...
        endpoint: endpoint || 'callJSON',
      });
    } catch (e) {
      console.error('[trackAnthropic-callJSON]', e.message);
    }
  }
