...
export async function getInstagramInsights(conn, mediaId) {
  // v22+ 廢掉 impressions 改用 views · reach/likes/comments/saved/shares/views 仍支援
  ...
  // 命名對齊:讓上層 impressions 欄位仍有值 (以 views 代)
  if (out.views != null && out.impressions == null) out.impressions = out.views;
  return out;
}
...
export async function getFacebookInsights(conn, postId) {
  // v22+ FB /post/insights 對單篇貼文能取的 metric 大幅收斂:
  // 只留 reactions + clicks · impressions/reach 需 App Review Advanced Access
  const metrics = 'post_reactions_by_type_total,post_clicks';
