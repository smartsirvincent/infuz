...
const POLL_MAX_ATTEMPTS = 60; // 60 * 3s = 3 分鐘 (V1 legacy)
const POLL_MAX_ATTEMPTS_V2 = 40; // V2 縮短為 40 * 3s = 2 分鐘,方便留時間重試 (Vercel 300s 上限)
...
export async function pollImageV2(taskId) {
  for (let i = 0; i < POLL_MAX_ATTEMPTS_V2; i++) {
    ...
    if (d.state === 'fail') {
      // 分辨可重試 (KIE 伺服器暫時性錯誤) vs 不可重試 (prompt 被封鎖等)
      const failMsg = d.failMsg || d.failCode || 'unknown';
      const retryable = /internal\s*error|timeout|temporarily|please try|server|503|502|500/i.test(failMsg);
      const err = new Error(`KIE V2 generation failed: ${failMsg} (taskId=${taskId})`);
      err.kieRetryable = retryable;
      err.kieTaskId = taskId;
      throw err;
    }
    await sleep(POLL_INTERVAL_MS);
  }
  // Poll timeout — 也是可重試(任務可能被 KIE 卡住,重送新任務通常更快)
  const err = new Error(`KIE V2 poll timeout (taskId=${taskId}, 已等 ${POLL_MAX_ATTEMPTS_V2 * POLL_INTERVAL_MS / 1000}s)`);
  err.kieRetryable = true;
  err.kieTaskId = taskId;
  throw err;
}
...
/**
 * V2 生圖 + 自動重試 (KIE 伺服器暫時性錯誤時)
 * maxRetries = 2 表示最多跑 3 次 (第 1 次 + 2 次重試)
 */
export async function submitAndPollV2WithRetry(opts, { maxRetries = 2, onRetry } = {}) {
  let lastErr;
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    try {
      const taskId = await submitImageV2(opts);
      const url = await pollImageV2(taskId);
      return { taskId, kieUrl: url };
    } catch (e) {
      lastErr = e;
      // 不可重試 (例:內容被封鎖) 直接 throw
      if (e.kieRetryable === false) throw e;
      // 到最後一次還失敗就 throw
      if (attempt === maxRetries) throw e;
      const backoff = 3000 * (attempt + 1); // 3s, 6s
      console.warn(`[KIE V2 retry ${attempt + 1}/${maxRetries}] ${e.message}, wait ${backoff}ms`);
      if (onRetry) onRetry({ attempt: attempt + 1, error: e });
      await sleep(backoff);
    }
  }
  throw lastErr;
