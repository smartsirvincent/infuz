...
async function publishSingle(token, userId, { text, imageUrl, replyToId, pollOptions }) {
  ...
  const params = {
    media_type: isPoll ? 'TEXT_POST' : imageUrl ? 'IMAGE' : 'TEXT',
    text,
    ...(imageUrl && !isPoll ? { image_url: imageUrl } : {}),
    ...(replyToId ? { reply_to_id: replyToId } : {}),
    // Poll: Threads Graph API 支援 2-4 個 options (2025-01 起) · 傳 JSON 字串陣列
    ...(isPoll ? { poll_options: JSON.stringify(pollOptions.slice(0, 4)) } : {}),
  };
  const container = await api(token, 'POST', `/${userId}/threads`, params);
  if (imageUrl && !isPoll) await waitContainerReady(token, container.id);
