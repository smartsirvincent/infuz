...
async function publishSingle(token, userId, { text, imageUrl, replyToId, pollOptions }) {
  const isPoll = Array.isArray(pollOptions) && pollOptions.length >= 2;
  // Threads API 2025-09 官方 poll 格式 (developers.facebook.com/documentation/threads/create-posts/polls):
  // - media_type 必須用 TEXT (不是 TEXT_POST)
  // - 參數名是 poll_attachment · 物件形式 {option_a, option_b, option_c?, option_d?}
  // - 每個選項 1-25 字元, 至少 2 個 (a+b 必填), 最多 4 個
  // - 只能掛在純文字貼文, 不能配圖片
  let pollAttachment = null;
  if (isPoll) {
    const opts = pollOptions.slice(0, 4).map((s) => String(s || '').trim().slice(0, 25)).filter(Boolean);
    if (opts.length >= 2) {
      pollAttachment = { option_a: opts[0], option_b: opts[1] };
      if (opts[2]) pollAttachment.option_c = opts[2];
      if (opts[3]) pollAttachment.option_d = opts[3];
    }
  }
  const params = {
    media_type: pollAttachment ? 'TEXT' : imageUrl ? 'IMAGE' : 'TEXT',
    text,
    // poll 只能純文字, 有 pollAttachment 就忽略 image_url
    ...(imageUrl && !pollAttachment ? { image_url: imageUrl } : {}),
    ...(replyToId ? { reply_to_id: replyToId } : {}),
    ...(pollAttachment ? { poll_attachment: JSON.stringify(pollAttachment) } : {}),
  };
  const container = await api(token, 'POST', `/${userId}/threads`, params);
  if (imageUrl && !pollAttachment) await waitContainerReady(token, container.id);
 ...
 * imageUrl = 只掛在第一段
 */
export async function publishThread(conn, chunks, imageUrl = null, pollOptions = null) {
  ...
  for (let i = 0; i < chunks.length; i += 1) {
    const result = await publishSingle(token, userId, {
      ...
      replyToId,
      // Poll 只掛在第一段 (parent post)
      pollOptions: i === 0 ? pollOptions : null,
    });
    posts.push(result);
    replyToId = result.id;
