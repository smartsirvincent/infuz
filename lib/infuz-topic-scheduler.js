...
import { notifyPublishFailure } from './infuz-notify.js';
    ...
    try {
      const res = await fetch(`${getBaseUrl()}/api/infuz/publish`, {
        ...
        body: JSON.stringify({
          ...
          pollOptions: post.pollOptions || null, // 投票文用
          skipNotify: true, // scheduler 自己送含主題名的通知
        }),
      });
      ...
      if (publishResult.ok) {
        ...
        notifyPublishFailure({
          source: `主題排程 · ${topic.name}`,
          post: { text: finalText, imageUrl: post.imageUrl, topicName: topic.name },
          results: publishResult.results || {},
        }).catch((e) => console.error('[scheduler/notify]', e.message));
      }
    } catch (err) {
      await updateItem('topic_posts', post.id, {
        status: 'failed',
  ...
  for (const post of due) {
    ...
    try {
      ...
      if (publishResult.ok) stats.fired++;
        ...
        stats.errors.push(`${post.id}: ${publishResult.error || 'failed'}`);
        notifyPublishFailure({
          source: `排程到期 (scheduledAt) · ${post.id}`,
          post: { text: finalText, imageUrl: post.imageUrl },
          results: publishResult.results || {},
        }).catch((e) => console.error('[scheduler/notify]', e.message));
      }
    } catch (err) {
