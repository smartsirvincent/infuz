...
import { FIDELITY_INSTRUCTION_FOR_CLAUDE, enforceFidelityPrompt, productReferenceUrls } from './infuz-image-rules.js';
import { stripMarkdown, stripBareImageUrls, NO_MARKDOWN_RULE } from './infuz-sanitize.js';
...
export const WEATHER_MODULE = {
  ...
  async buildContent({ config, snapshot, snapshots, withImage, dryRunImage = false }) {
...
${NO_MARKDOWN_RULE}
${withImage ? FIDELITY_INSTRUCTION_FOR_CLAUDE : ''}`;
...
請回傳 JSON:
{
  ...
  "hashtags": "3-6 個相關 hashtag(#開頭,空白分隔)"
    ...
    // dryRunImage=true(preview 用): 只回 imagePrompt 不真的生圖(避免 Vercel Hobby 60s 上限)
    let imageUrl = null;
    let kieTaskId = null;
    let imageError = null;
    if (withImage && result.imagePrompt && !dryRunImage) {
      try {
        const refs = productReferenceUrls(picked); // 多角度參考照
        const finalPrompt = enforceFidelityPrompt(result.imagePrompt);
    ...
    return {
      text: stripBareImageUrls(stripMarkdown((result.text || '').trim())),
      imagePrompt: withImage ? (result.imagePrompt || imagePromptGuide) : null, // imagePrompt 不 strip (英文 prompt 沒 markdown 問題)
      hashtags: stripBareImageUrls(stripMarkdown(result.hashtags || '')),
      imageUrl,
      kieTaskId,
      imageError,
...
async function pickProduct(config) {
  try {
    ...
    // 珠寶不參與穿搭(視覺不合)
    const clothing = items.filter((p) => p.category !== '珠寶' && p.image_front && !p.paused);
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      if (publishResult.ok) stats.fired++;
        ...
        // 送 email 通知 (dynamic import 避免 top-level 影響 module 讀取)
        try {
          const { notifyPublishFailure } = await import('./infuz-notify.js');
          notifyPublishFailure({
            source: `氣候即時 · ${job.name}`,
            post: { text: finalText, imageUrl: built.imageUrl, topicName: job.name },
            results: publishResult.results || {},
          }).catch((e) => console.error('[realtime/notify]', e.message));
        } catch (_) { /* ignore */ }
      }
    } catch (err) {
      await updateItem('realtime_jobs', job.id, {
