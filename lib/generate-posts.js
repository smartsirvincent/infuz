...
**重要**: product_index 必須是上方清單中的 # (允許值: ${allowedIndices.join(', ')})。${products.length > 1 ? '這批要盡量輪替不同 SKU。' : ''}`,
  ...
  // 文體 style override (從 theme.style 來,由 recommend 階段分配)
  const styleOverride = buildStyleOverride(theme.style);

  // 主題自訂提示 (Step 2 用戶編輯 theme.custom_hint,優先級最高)
  const customHintBlock = (theme.custom_hint || '').trim()
    ? `\n\n**🎯 用戶自訂主題提示 (最高優先級,若與上面文體規則衝突以此為準)**:\n${theme.custom_hint.trim()}`
    : '';
...
/**
 * 依文體 style 加進一步寫作約束,讓「對白文」真的是對白、「自言自語」真的破碎
 */
function buildStyleOverride(style) {
  if (!style) return '';
  const map = {
    '對白文': `
7. **【文體強制:純對話】** 文案內容必須是純對話格式,不能有敘事/旁白。
   - 格式:每行用「角色名:內容」開頭,角色之間用 \\n 分隔
   - 例:
     妹妹:這件褲子哪買的
     我:我都不告訴妳
     妹妹:...
     我:好啦我傳給妳
   - 兩到三個角色之間的真實對話,口語、有節奏、來回 5-10 句
   - **不可** 在對話之外加「她說/我想/旁邊的人...」這種旁白
   - **不可** 用引號 (「」) 包裝對話,直接 "角色:內容" 換行就好`,
    '情境敘事': `
7. **【文體:情境敘事】** 第一人稱當下感「我...」,像在記錄正在發生的事。
   - 短句、換行多、有畫面感
   - 不要用「以前我...」這種回憶句,而是「現在 / 剛才 / 此刻...」
   - 結尾不一定要 CTA,可留白`,
    '自言自語': `
7. **【文體:自言自語】** 內心戲、句子破碎、像深夜筆記。
   - 不完整的句子可以,刪節號可以,半截想法可以
   - 不用對讀者解釋,自己跟自己說話
   - 不要結論句,留懸念`,
    '客戶見證': `
7. **【文體:客戶見證】** 引述客人的話,而不是品牌方說話。
   - 開頭可以:「客人 OO 說:」「上次有個 25 歲的女孩告訴我:」「FB 私訊有人寫:」
   - 用引號或換行區分「引述」和「品牌補充」
   - 結尾品牌可以輕補一句,不超過 1-2 行`,
    '觀點短文': `
7. **【文體:觀點短文】** 結論先行,1-2 句解釋,結束。
   - 開頭就放結論/反直覺觀點
   - 解釋不超過 3 行
   - 句子精煉,不要囉嗦`,
    '教學文': `
