...
export default function ImagePlanPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await loadInfuzInput({ generate_images: true });
        setInput(data);
      } catch (e) {
  ...
  return (
    <main className="space-y-6">
      <div className="card border-purple-200 bg-purple-50/40">
        <h1 className="flex items-center gap-2 text-xl font-semibold text-stone-900">🖼️ 圖片貼文（Infuz）</h1>
        <p className="mt-1 text-sm text-stone-600">
          已自動載入 Infuz 品牌資料 + <strong>{input.products?.length || 0}</strong> 個 SKU。直接走「下一步」AI 推薦圖片主題。
        </p>
